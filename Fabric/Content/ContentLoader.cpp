...
namespace fabric::content
{
	namespace
	{
		...
		bool read_transform(const u8*& data, entity::entity_info& info)
		{
		...
		bool read_script(const u8*& data, entity::entity_info& info)
		{
			assert(!info.script);
		...
		using component_reader = bool(*)(const u8*&, entity::entity_info&);
	...
	bool load_game()
	{
		...
		for (u32 entity_index{ 0 }; entity_index < num_entities; ++entity_index)
		{
			...
			assert(info.transform);
			entity::entity entity{ entity::create(info) };
