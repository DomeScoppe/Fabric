...
namespace fabric::script
{
    ...
    namespace detail
    {
        ...
        {
            auto script = fabric::script::registry().find(tag);
            assert(script != fabric::script::registry().end() && script->first == tag);
...
{
    const u32 size{ (u32)fabric::script::script_names().size() };
    if (!size) return nullptr;
    CComSafeArray<BSTR> names(size);
    for (u32 i{ 0 }; i < size; i++)
    {
        names.SetAt(i, A2BSTR_EX(fabric::script::script_names()[i].c_str()), false);
    }
